<script lang="ts">
  ...
  import {
    ...
    createMintTokenTx,
    createSplTransferTx,
  } from "../utils/tokenProgram2022";
  import { signAndSendTx } from "../utils/web3Authorize";
  ...
  // Create Mint
  const createMint = async (wallet: WalletStore, connection: Connection) => {
    // Create tx to create mint and sign with wallet and send Transaction
    let configData = new MintConfigData(
      decimals,
      isCloseAuthority,
      isNonTransferable,
      feeBasisPoints,
  ...
  const mintTokenToAccount = async (
    ...
    let tx = await createMintTokenTx(
      connection,
      mint,
      ata,
      wallet.publicKey!,
  ...
  const transferToken = async (
    ...
    let tx = await createSplTransferTx(
      connection,
      mint,
      wallet.publicKey!,
      tokenAmount,
  ...
  <div class="grid grid-cols-1 space-y-8">
    ...
    <div>
      ...
      <div class="grid grid-cols-2 gap-x-4">
        <div>
          ...
          <input
            ...
            max="9"
            bind:value={decimals}
            placeholder="Enter decimals (Optional) ..."
          />
        </div>

        {#if programId == TOKEN_2022_PROGRAM_ID}
          <div>
            <label for="Fee Basis Points" class="text-xs"
              >Fee Basis Points</label
            >
            <input
              class="w-full text-black"
              type="number"
              ...
              step="100"
              max="10000"
              bind:value={feeBasisPoints}
              placeholder="Enter fee basis points (Optional) ..."
            />
          </div>
          <div>
            ...
            <input
              ...
              min="0"
              bind:value={maxFee}
              placeholder="Enter max. fee (Optional) ..."
            />
          </div>
          <div>
          ...
          <div class="flex flex-wrap items-center space-x-2">
            <div
              class="flex items-center border border-gray-200 rounded dark:border-gray-700"
            >
              <input
                type="checkbox"
                bind:checked={isNonTransferable}
            ...
            >
              <input
                type="checkbox"
                bind:checked={isCloseAuthority}
                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
              />
              <label
                for="bordered-checkbox-2"
                class="w-full py-2 ml-2 text-sm font-medium text-gray-900 dark:text-gray-300"
                >Close Authority</label
              >
            </div>
          </div>
        {/if}
      ...
      <div class="mt-4">
        <Button
          label="Create Mint"
          onClick={() => createMint($walletStore, $connectedCluster)}
        />
      </div>
        ...
        {:then value}
          {#if value}
            <a
              ...
              href="https://solscan.io/tx/{value}?cluster={$cluster}"
              target="_blank"
    ...
    {#if programId == TOKEN_2022_PROGRAM_ID}
      <div class="grid grid-cols-1 space-y-4">
        ...
        <div>
          <Button
            label="Close Mint"
            onClick={() => closeMint($walletStore, $connectedCluster, mint)}
          />
        </div>
        <div>
          <!-- Response Output -->
    ...
    <div class="grid grid-cols-1 space-y-4">
      <div>
        ...
        <input
          class="w-full text-black"
          bind:value={tokenOwner}
          placeholder="Enter recipient token owner address ..."
        />
      </div>
      <div>
        <Button
        ...
        <Button
          label="Copy Wallet"
          styling="bg-blue-600"
          onClick={() => (tokenOwner = $walletStore.publicKey || "")}
        />
        <!-- Response Output -->
        <div>
    ...
    <div>
      ...
      <div>
        ...
        <input
          ...
          bind:value={ata}
          placeholder="Enter recipient associated token account address ..."
        />
        ...
        />
        <Button
          label="Mint Token to Token Account"
          onClick={() =>
            mintTokenToAccount(
              $walletStore,
              $connectedCluster,
    ...
    <div>
      ...
      <div>
        ...
        <input
          class="w-full mb-4 text-black"
          bind:value={recipientOfTransfer}
        ...
        <Button
          label="Copy Wallet"
          styling="bg-blue-600"
          onClick={() => (recipientOfTransfer = $walletStore.publicKey || "")}
        />
        <Button
          label="Copy Ata"
          styling="bg-blue-600"
          onClick={() => (recipientOfTransfer = ata)}
        />
        <!-- Response Output -->
        <div>
